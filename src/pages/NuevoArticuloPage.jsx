...
const NuevoArticuloPage = () => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        toast({
          ...
          description: 'El archivo debe ser formato Word, RTF, OpenOffice o WordPerfect.',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
  ...
  const validateForm = () => {
    ...
    if (!formData.resumen.trim()) {
      ...
      newErrors.resumen = 'El resumen no puede exceder 250 palabras';
    }
    
    if (!formData.palabrasClave.trim()) {
      newErrors.palabrasClave = 'Las palabras clave son obligatorias';
    }
    ...
    // Verificar que todos los elementos de la lista de comprobación estén marcados
    const incompleteItems = Object.entries(checklist)
      .filter(([_, checked]) => !checked)
      .map(([key, _]) => key);
    
    if (incompleteItems.length > 0) {
      newErrors.checklist = 'Debe completar todos los elementos de la lista de comprobación';
    }
          ...
          <Collapse in={isChecklistOpen}>
            <CardBody pt={0}>
              ...
              <VStack align="start" spacing={3}>
                ...
                >
                  <Text fontSize="sm">
                    El texto tiene interlineado sencillo, tamaño fuente de 12 puntos, se utiliza cursiva en lugar de subrayado 
                    (excepto en las direcciones URL), y todas las ilustraciones, figuras y tablas están colocadas en los lugares apropiados.
                  </Text>
                </Checkbox>
                ...
                >
                  <Text fontSize="sm">
                    Para artículos de evaluación por pares, se siguen las instrucciones para asegurar una evaluación anónima.
                  </Text>
                </Checkbox>
              </VStack>
